...
DIRECTORY_PATTERN = 'layer[1-7]'
ATOMICS_DIR_RELATIVE_PATH = os.path.join(os.getcwd(), "techniques")
...
def get_yaml_files_from_dir(path_to_dir):
    """Returns path of the all files that matches "*.yaml" in a directory."""
    ...
    for entry in os.listdir(path_to_dir):
        ...
        if fnmatch.fnmatch(entry, '*.yaml'):
            # Found the file!
            file_entries.append(os.path.join(path_to_dir, entry))
...
def load_technique(file_entry):
    """Loads the YAML content of a technique from its directory. """
...
def load_techniques():
    ...
    for atomic_entry in os.listdir(normalized_atomics_path):
        ...
        if fnmatch.fnmatch(atomic_entry, DIRECTORY_PATTERN):
            
            # Get path to tech dir.
            path_to_dir = os.path.join(normalized_atomics_path, atomic_entry)
            # Get all techniques on the path
            technique_files = get_yaml_files_from_dir(path_to_dir)
            for technique_file in technique_files:
                # Load, parse and add to dict.
                tech = load_technique(technique_file)
                print(", {}".format(tech['attack_technique']), end="")
                techniques[tech['attack_technique']] = tech
                
                # Add path to technique's directory.
                techniques[tech['attack_technique']]["path"] = path_to_dir
        print(".")
    return techniques
...
def print_non_interactive_command_line(technique_name, executor_number, parameters, check_dep, run_cleanup):
    ...
    print("    Python:")
    print("    techniques = runner.Runner()")
...
class Runner():
    ...
    def repl(self):
        ...
        print("Enter the name of the technique that you would like to execute (eg. icmp_flood).  Type 'exit' to quit.")
        i = input("> ").strip()
